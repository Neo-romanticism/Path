  ...
  if (u.departments != null) {
    ...
    for (let j = 0; j < u.departments.length; j += 1) {
      ...
      if (typeof d.name !== 'string' || !d.name.trim())
        fail(`row ${i + 1} dept ${j + 1}: name is required`);
      if (typeof d.category !== 'string' || !d.category.trim())
        fail(`row ${i + 1} dept ${j + 1}: category is required`);
      ...
      for (const [trackName, trackData] of Object.entries(admissions)) {
        ...
        if (!hasScore) {
          fail(
            `row ${i + 1} dept ${j + 1} track ${trackName}: at least one score field is required`,
          );
        }
        ...
        if (!Number.isFinite(confidence) || confidence < MIN_CONFIDENCE || confidence > 1) {
          fail(
            `row ${i + 1} dept ${j + 1} track ${trackName}: confidence must be ${MIN_CONFIDENCE}~1`,
          );
        }
      }
    }
