...
async function main() {
  ...
  const role = legacyTrue ? 'sub' : legacyFalse ? 'none' : mode;
  ...
  try {
    ...
    await pool.query('UPDATE users SET is_admin = $1, admin_role = $2 WHERE id = $3', [
      isAdmin,
      role,
      target.id,
    ]);
    ...
    console.log(
      `Updated: ${target.nickname} (id=${target.id}) -> role=${role}, is_admin=${isAdmin}`,
    );
