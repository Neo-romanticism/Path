...
function importRowsToPipeline(pipeline, rows, options) {
  const { defaultSourceId, defaultYear, mapSpec, fallbackSourceUrl, replace } = options;
...
function cmdSeedFromBuiltin(args) {
  ...
  savePipeline(pipeline);
  console.log(
    `[ok] seeded ${result.imported}/${rows.length} rows from built-in dataset (source=${sourceId}, year=${defaultYear})`,
  );
}
...
async function cmdCollect(args) {
  const manifestPath = args.manifest ? path.resolve(args.manifest) : SOURCE_MANIFEST_PATH;
  ...
  for (const src of manifest.sources) {
    ...
    console.log(
      `[ok] ${id}: imported=${result.imported}, skipped=${result.skipped}, snapshot=${snapshotPath}`,
    );
  }
  ...
  console.log(
    `[ok] collect complete: sources=${totalSources}, imported=${totalImported}, skipped=${totalSkipped}, dryRun=${dryRun}`,
  );
}

function evaluateTrust(rec, policy) {
  const reasons = [];
  const minConfidence = Number.isFinite(Number(policy.minConfidence))
...
function cmdSetTrustPolicy(args) {
  ...
  const next = {
    ...
    requireYear:
      args.requireYear != null ? String(args.requireYear) !== 'false' : current.requireYear,
    requireSourceId:
      args.requireSourceId != null
        ? String(args.requireSourceId) !== 'false'
        : current.requireSourceId,
    requireSourceUrl:
      args.requireSourceUrl != null
        ? String(args.requireSourceUrl) !== 'false'
        : current.requireSourceUrl,
    requireAtLeastOneScore:
      args.requireAtLeastOneScore != null
