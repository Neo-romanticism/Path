...
router.delete('/calendar/plan/:id', async (req, res) => {
  ...
  try {
    await pool.query('DELETE FROM study_plans WHERE id = $1 AND user_id = $2', [
      id,
      req.session.userId,
    ]);
    res.json({ ok: true });
...
router.put('/calendar/plan/:id', async (req, res) => {
  ...
  const subjectIdRaw = req.body.subject_id;
  const subjectId =
    subjectIdRaw === undefined || subjectIdRaw === null || subjectIdRaw === ''
      ? null
      : parseInt(subjectIdRaw, 10);
  const planDate = String(req.body.plan_date || '').trim();
  const startMinute = parseTimeToMinute(req.body.start_time);
  const endMinute = parseTimeToMinute(req.body.end_time);
  const note =
  ...
  if (subjectIdRaw !== undefined && !subjectId) {
    return res.status(400).json({ error: '유효한 과목을 선택하세요.' });
  }
  ...
  try {
    ...
    let nextSubjectId = prev.subject_id;
    if (subjectId) {
      const ownsSubject = await pool.query(
...
router.post('/start', async (req, res) => {
  ...
  const target = studyMode === 'timer' ? Math.min(rawTargetSec, 86400) : 0;
  ...
  try {
    ...
    const recordInsertRes = await client.query(
      ...
      [
        ...
        earnedGold,
        earnedExp,
        user.current_study_subject_id || null,
      ],
    );
    ...
    const updRes = await client.query(
      `UPDATE users
             ...
             WHERE id = $3
             RETURNING id, nickname, university, gold, diamond, exp, tier, tickets, is_studying, active_title, streak_count, streak_last_date`,
    ...
    if (bonusGold > 0) {
      const userRes = await client.query(
        `UPDATE users
                 SET gold = gold + $1
                 WHERE id = $2
                 RETURNING id, nickname, university, gold, diamond, exp, tier, tickets, is_studying`,
...
router.get('/proof-image/:filename', async (req, res) => {
  ...
  try {
    ...
    const adminCheck = await pool.query('SELECT is_admin FROM users WHERE id = $1', [
      req.session.userId,
    ]);
...
router.get('/stats', async (req, res) => {
  ...
  try {
    const userResult = await pool.query(
      'SELECT id, nickname, university, gold, diamond, exp, tier, tickets, is_studying, active_title, streak_count, streak_last_date FROM users WHERE id = $1',
      [req.session.userId],
    );
