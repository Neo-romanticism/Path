...
router.get('/settings', requireAuth, async (req, res) => {
  try {
    const result = await pool.query('SELECT cam_enabled, cam_visibility FROM users WHERE id = $1', [
      req.session.userId,
    ]);
...
router.post('/settings', requireAuth, async (req, res) => {
  ...
  try {
    await pool.query('UPDATE users SET cam_enabled = $1, cam_visibility = $2 WHERE id = $3', [
      !!cam_enabled,
      vis,
      req.session.userId,
    ]);
...
router.post('/upload', requireAuth, async (req, res) => {
  ...
  try {
    const userResult = await pool.query('SELECT cam_enabled FROM users WHERE id = $1', [
      req.session.userId,
    ]);
...
router.get('/recent', requireAuth, async (req, res) => {
  try {
    const isAdmin = await pool.query('SELECT is_admin FROM users WHERE id = $1', [
      req.session.userId,
    ]);
