...
router.post('/attack', async (req, res) => {
  ...
  if (parseInt(defender_id) === req.session.userId)
    return res.status(400).json({ error: '자신에게 지원할 수 없습니다.' });
  ...
  try {
    ...
    if (attacker.tickets < 1) {
      await client.query('ROLLBACK');
      return res.status(400).json({ error: '원서비가 없습니다.' });
    }
    ...
    if (
      !attackerScores?.korean_std ||
      !attackerScores?.math_std ||
      !attackerScores?.explore1_std ||
      !attackerScores?.explore2_std
    ) {
      await client.query('ROLLBACK');
    ...
    if (usedSlots >= MAX_DAILY) {
      ...
      return res.status(400).json({
        error: `오늘 지원 가능 횟수(${MAX_DAILY}회)를 모두 사용했습니다. 내일 다시 도전하세요.`,
      });
    }
    ...
    await client.query(
      ...
      [
        req.session.userId,
        defender_id,
        Math.round(Number(kanInfo.userScore || 0)),
        Math.round(acceptProb * 100),
        invasionResult,
        0,
      ],
    );
...
router.get('/accept-prob', async (req, res) => {
  ...
  try {
    ...
    if (
      !userScores?.korean_std ||
      !userScores?.math_std ||
      !userScores?.explore1_std ||
      !userScores?.explore2_std
    ) {
...
router.get('/my-applications', async (req, res) => {
  if (!req.session.userId) return res.status(401).json({ error: '로그인이 필요합니다.' });
  try {
    const userRes = await pool.query('SELECT score_status, tickets FROM users WHERE id = $1', [
      req.session.userId,
    ]);
