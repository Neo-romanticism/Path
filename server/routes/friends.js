...
router.post('/request', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const targetRes = await pool.query('SELECT allow_friend_requests FROM users WHERE id = $1', [
      target_id,
    ]);
    ...
    const senderRes = await pool.query('SELECT nickname FROM users WHERE id = $1', [
      req.session.userId,
    ]);
    await pool.query('INSERT INTO notifications (user_id, type, message) VALUES ($1, $2, $3)', [
      target_id,
      'friend_request',
      `${senderRes.rows[0]?.nickname}님이 동맹 신청을 보냈습니다.`,
    ]);
...
router.post('/accept', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const myRes = await pool.query('SELECT nickname FROM users WHERE id = $1', [
      req.session.userId,
    ]);
    await pool.query('INSERT INTO notifications (user_id, type, message) VALUES ($1, $2, $3)', [
      result.rows[0].sender_id,
      'friend_accept',
      `${myRes.rows[0]?.nickname}님이 동맹 신청을 수락했습니다.`,
    ]);
