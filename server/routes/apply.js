...
function collectGroupTokens(bucket, value) {
  ...
  const tokens = text.replace(/[|,/]/g, ' ').replace(/\s+/g, ' ').trim().split(' ').filter(Boolean);
  ...
  if (err instanceof multer.MulterError) {
    if (err.code === 'LIMIT_FILE_SIZE') {
      return res
        .status(413)
        .json({
          error: `이미지 용량은 최대 ${Math.floor(SCORE_IMAGE_MAX_SIZE / (1024 * 1024))}MB까지 가능합니다.`,
        });
    }
  ...
  try {
    await pool.query(
      ...
      [
        req.session.userId,
        korean_std || null,
        korean_percentile || null,
        korean_subject || null,
        math_std || null,
        math_percentile || null,
        math_subject || null,
        english_grade || null,
        explore1_subject || null,
        explore1_std || null,
        explore1_percentile || null,
        explore2_subject || null,
        explore2_std || null,
        explore2_percentile || null,
        history_grade || null,
        second_lang_subject || null,
        second_lang_std || null,
        second_lang_percentile || null,
        source_round_name || null,
      ],
    );

    const row = await pool.query('SELECT * FROM exam_scores WHERE user_id = $1', [
      req.session.userId,
    ]);
...
router.get('/scores/me', requireAuth, async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM exam_scores WHERE user_id = $1', [
      req.session.userId,
    ]);
...
router.get('/kan', requireAuth, async (req, res) => {
  ...
  try {
    const scoreRes = await pool.query('SELECT * FROM exam_scores WHERE user_id = $1', [
      req.session.userId,
    ]);
...
router.get('/kan/:userId', requireAuth, async (req, res) => {
  ...
  if (targetId === req.session.userId)
    return res.status(400).json({ error: '본인 칸수는 무료로 조회하세요.' });
  if (!university) return res.status(400).json({ error: '대학명을 입력해주세요.' });
...
router.post('/applications', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (round.status !== 'open') {
      await client.query('ROLLBACK');
      return res.status(400).json({ error: '지원 기간이 아닙니다.' });
    }

    // 점수 인증 여부 (점수 입력은 필수, 인증은 선택이지만 점수 입력은 있어야 함)
    const scoreRes = await client.query(`SELECT korean_std FROM exam_scores WHERE user_id = $1`, [
      req.session.userId,
    ]);
    ...
    res.json({
      ...
      remaining_tickets: ticketRes.rows[0].tickets,
    });
  } catch (err) {
    await client.query('ROLLBACK');
...
router.delete('/applications/:id', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!app) {
      await client.query('ROLLBACK');
      return res.status(404).json({ error: '원서를 찾을 수 없습니다.' });
    }
    if (app.status === 'cancelled') {
      await client.query('ROLLBACK');
      return res.status(400).json({ error: '이미 취소된 원서입니다.' });
    }
    if (app.round_status !== 'open') {
      await client.query('ROLLBACK');
      return res.status(400).json({ error: '취소 가능한 기간이 아닙니다.' });
    }
    ...
    // 티켓 환불
    await client.query('UPDATE users SET tickets = tickets + 1 WHERE id = $1', [
      req.session.userId,
    ]);
...
router.get('/search', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const unis = getAllUniversities().filter((u) => {
      const nameHit = String(u.name || '')
        .toLowerCase()
        .includes(qLower);
      const nameNormalizedHit = normalizeSearchText(u.name).includes(qNormalized);
      const aliasHit = (u.aliases || []).some((a) =>
        String(a || '')
          .toLowerCase()
          .includes(qLower),
      );
      const aliasNormalizedHit = (u.aliases || []).some((a) =>
        normalizeSearchText(a).includes(qNormalized),
      );
      ...
      return fullUni.departments.some(
        (dept) =>
          String(dept?.name || '')
            .toLowerCase()
            .includes(qLower) || normalizeSearchText(dept?.name || '').includes(qNormalized),
      );
    });
    ...
    for (const uniMeta of unis.slice(0, 20)) {
      ...
      const matchedDepts = deptList.filter((dept) => {
        ...
        const deptNameHit = String(dept?.name || '')
          .toLowerCase()
          .includes(qLower);
        const uniNameHit = String(fullUni.name || '')
          .toLowerCase()
          .includes(qLower);
...
router.post('/enroll', requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (!app) {
      await client.query('ROLLBACK');
      return res.status(404).json({ error: '합격한 원서를 찾을 수 없습니다.' });
    }
    ...
    // 선택 원서 enrolled 처리
    await client.query(`UPDATE applications SET status = 'enrolled' WHERE id = $1`, [
      application_id,
    ]);

    // 대학 이전
    await client.query(`UPDATE users SET university = $1 WHERE id = $2`, [
      app.university,
      req.session.userId,
    ]);
    ...
    await client.query(
      ...
      [
        req.session.userId,
        `🎓 ${app.university}${app.department ? ' ' + app.department : ''} 최종 등록 완료!`,
      ],
    );
