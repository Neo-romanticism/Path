...
router.get('/compare-gpa', (req, res) => {
  ...
  if (!university || !gpa)
    return res.status(400).json({ error: '대학명과 내신 등급을 입력해주세요.' });

  const myGpa = parseFloat(gpa);
  if (isNaN(myGpa) || myGpa < 1 || myGpa > 9)
    return res.status(400).json({ error: '내신 등급은 1.0~9.0 사이입니다.' });

  const info = getUniversityInfo(university);
  if (!info) return res.json({ found: false, university });

  const results = info.departments
    .map((dept) => {
      const comparisons = {};
