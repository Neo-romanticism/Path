...
router.post('/', roomsWriteLimiter, requireAuth, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: '초대 코드 생성에 실패했습니다. 잠시 후 다시 시도해주세요.' });
  } catch (err) {
...
router.delete('/:id/leave', roomsWriteLimiter, requireAuth, async (req, res) => {
  ...
  try {
    await Promise.all([
      pool.query(`DELETE FROM study_room_member_roles WHERE room_id = $1 AND user_id = $2`, [
        roomId,
        req.session.userId,
      ]),
      pool.query(`DELETE FROM study_room_members WHERE room_id = $1 AND user_id = $2`, [
        roomId,
        req.session.userId,
      ]),
    ]);
...
router.patch('/:id', roomsWriteLimiter, requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (maxMembers < parseInt(check.rows[0].member_count, 10))
      return res
        .status(400)
        .json({ error: '현재 멤버 수보다 최대 인원을 작게 설정할 수 없습니다.' });
...
router.delete('/:id/members/:userId', roomsWriteLimiter, requireAuth, async (req, res) => {
  ...
  try {
    ...
    await client.query(`DELETE FROM study_room_member_roles WHERE room_id = $1 AND user_id = $2`, [
      roomId,
      targetUserId,
    ]);
    await client.query(`DELETE FROM study_room_members WHERE room_id = $1 AND user_id = $2`, [
      roomId,
      targetUserId,
    ]);
...
router.patch('/:id/owner', roomsWriteLimiter, requireAuth, async (req, res) => {
  ...
  try {
    ...
    await client.query(`UPDATE study_rooms SET creator_id = $2 WHERE id = $1`, [
      roomId,
      nextOwnerUserId,
    ]);
    await client.query(
    ...
    res.json({
      ok: true,
      room_id: roomId,
      previous_owner_id: req.session.userId,
      new_owner_id: nextOwnerUserId,
    });
  } catch (err) {
    if (client) await client.query('ROLLBACK').catch(() => {});
  ...
  try {
    ...
    if (hardDelete) {
      if (!confirmName || confirmName !== roomName) {
        ...
        return res
          .status(400)
          .json({ error: '하드 삭제는 confirm_name에 정확한 방 이름이 필요합니다.' });
      }
...
router.post('/:id/messages', chatLimiter, requireAuth, async (req, res) => {
  ...
  try {
    ...
    const userRes = await pool.query(`SELECT nickname FROM users WHERE id = $1`, [
      req.session.userId,
    ]);
...
router.get('/:id/decor', roomsReadLimiter, requireAuth, async (req, res) => {
  ...
  try {
    ...
    const [ownedRes, stateRes] = await Promise.all([
      ...
      pool.query(`SELECT wallpaper_key, prop_keys FROM room_decor_state WHERE room_id = $1`, [
        roomId,
      ]),
    ]);
...
router.post('/:id/shop/buy', roomsWriteLimiter, requireAuth, async (req, res) => {
  ...
  try {
    ...
    if (alreadyOwned.rows.length)
      return res.status(400).json({ error: '이미 구매된 아이템입니다.' });
    ...
    if (!goldRes.rows.length) {
      ...
      return res
        .status(400)
        .json({ error: `골드가 부족합니다. ${item.price.toLocaleString()}G 필요` });
    }
...
router.post('/:id/decor/equip', roomsWriteLimiter, requireAuth, async (req, res) => {
  ...
  try {
    ...
    const ownedRes = await pool.query(`SELECT item_key FROM room_owned_items WHERE room_id = $1`, [
      roomId,
    ]);
    const owned = new Set(ownedRes.rows.map((r) => r.item_key));
    owned.add('default');

    const newWallpaper = typeof wallpaper === 'string' && owned.has(wallpaper) ? wallpaper : null;
    ...
    if (newWallpaper !== null) {
      sets.push(`wallpaper_key = $${vals.push(newWallpaper)}`);
    }
    if (newProps !== null) {
      sets.push(`prop_keys = $${vals.push(JSON.stringify(newProps))}`);
    }
