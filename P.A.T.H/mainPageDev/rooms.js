...
(function () {
  ...
  const GroupRooms = {
    ...
    renderRoomList() {
      ...
      container.innerHTML = this.myRooms
        .map((room) => {
    ...
    async loadPublicRooms(append = false) {
      ...
      try {
        ...
        if (listEl)
          listEl.innerHTML =
            '<div class="rooms-empty"><div class="rooms-empty-icon">😕</div><div class="rooms-empty-text">불러올 수 없어요</div></div>';
      } finally {
    ...
    renderPublicRooms() {
      ...
      listEl.innerHTML = this._publicRooms
        .map((room, i) => {
          const members = parseInt(room.member_count, 10) || 0;
          ...
          const rankBadge =
            isSortByStudy && i < 3
              ...
              : isSortByStudy
                ? `<span class="room-rank-badge rank-n">#${i + 1}</span>`
                : '';
    ...
    renderDonutChart(members) {
      ...
      if (total === 0) {
        ...
        if (centerEl)
          centerEl.innerHTML =
            '<div class="room-donut-center-val">—</div><div class="room-donut-center-label">공부 기록 없음</div>';
        if (legendEl)
          legendEl.innerHTML = '<div class="donut-empty-msg">오늘 공부 기록이 없어요</div>';
        return;
      }

      // Build SVG segments (r=15.9155 → circumference≈100)
      let cumPct = 0;
      segEl.innerHTML = active
        .map((m, i) => {
    ...
    renderWeeklyChart(weekly) {
      ...
      const map = {};
      weekly.forEach((r) => {
        map[r.day] = parseInt(r.total_sec, 10);
      });

      const maxSec = Math.max(...days.map((d) => map[d.key] || 0), 1);
      const todayKey = new Date().toISOString().slice(0, 10);

      el.innerHTML = `<div class="weekly-bars">${days
        .map((d) => {
    ...
    showRoomView(roomId) {
      ...
      if (room) {
        ...
        document
          .getElementById('room-settings-creator-section')
          ?.classList.toggle('hidden', !canEditSettings);
        document
          .getElementById('room-settings-delete-btn')
          ?.classList.toggle('hidden', !canDeleteRoom);
    ...
    async shareInviteLink() {
      ...
      const roomName =
        document.getElementById('room-view-name')?.textContent?.trim() || '그룹 타이머';
    ...
    _renderSettingsMemberControls() {
      ...
      if (!members.length) {
        wrap.innerHTML =
          '<div class="room-settings-group"><div class="room-settings-group-label">멤버 관리</div><div class="room-settings-empty">멤버 정보를 불러오는 중입니다.</div></div>';
        return;
      }
                    ...
                    <div class="room-member-admin-list">
                        ${members
                          .map((m) => {
                            ...
                            const canKick =
                              !isMe && role !== 'owner' && (isOwner || role === 'member');
    ...
    async transferOwner(userId) {
      if (!this.activeRoomId) return;
      if (!confirm('정말 방장을 위임할까요? 위임 후에는 방 삭제/역할 부여 권한이 사라집니다.'))
        return;
      try {
        await this.apiPatch(`/api/rooms/${this.activeRoomId}/owner`, {
          next_owner_user_id: userId,
        });
        await Promise.all([this.loadRoomContext(this.activeRoomId), this.loadMyRooms()]);
    ...
    _applyRoomVisual() {
      ...
      if (propsEl) {
        const activeProps = (this._decor.props || [])
          .map((k) => ROOM_SHOP.props.find((p) => p.key === k))
          .filter(Boolean);
        propsEl.innerHTML = activeProps
          .map((p, i) => {
    ...
    switchDecorTab(tab) {
      ...
      document
        .querySelectorAll('.room-decor-panel')
    ...
    _renderWallpaperTab() {
      ...
      grid.innerHTML = ROOM_SHOP.wallpapers
        .map((w) => {
    ...
    async _equipWallpaper(key) {
      if (!this.activeRoomId) return;
      try {
        await this.apiPost(`/api/rooms/${this.activeRoomId}/decor/equip`, {
          wallpaper: key,
          props: this._decor.props,
        });
    ...
    _renderPropsTab() {
      ...
      grid.innerHTML = ROOM_SHOP.props
        .map((p) => {
    ...
    async _onPropClick(key, isOwned, isOn) {
      ...
      if (isOn) {
        ...
        if (props.length >= 9) {
          showToast('소품은 최대 9개까지 배치할 수 있어요');
          return;
        }
        props.push(key);
      }
      try {
        await this.apiPost(`/api/rooms/${this.activeRoomId}/decor/equip`, {
          wallpaper: this._decor.wallpaper,
          props,
        });
    ...
    async _buyItem(key) {
      ...
      try {
        const data = await this.apiPost(`/api/rooms/${this.activeRoomId}/shop/buy`, {
          item_key: key,
        });
        // Update gold in UI
        if (typeof UI !== 'undefined' && UI.currentUser) {
          UI.currentUser.gold = data.new_gold;
          if (UI.elements && UI.elements.goldVal)
            UI.elements.goldVal.textContent = data.new_gold.toLocaleString();
          const goldEl = document.getElementById('room-decor-gold-val');
    ...
    _getDecorSummary() {
      ...
      const rarityWeighted = propKeys.reduce(
        (sum, key) => sum + (PROP_RARITY_WEIGHTS[key] || 1),
        0,
      );
    ...
    _renderDecorShowcase() {
      ...
      const rankText = stats.myRank
        ? `내 순위 ${stats.myRank}/${stats.memberCount}`
        : '내 순위 집계 중';
    ...
    async _generateShowcaseCardBlob() {
      ...
      ctx.fill();
      ctx.beginPath();
      ctx.arc(920, 260, 90, 0, Math.PI * 2);
      ctx.fill();
      ctx.globalAlpha = 1;
      ...
      ctx.fillText(
        `테마: ${data.wallpaperName} · 소품 ${data.propCount}개 · 희소도 ${data.rarityWeighted}`,
        150,
        490,
      );
    ...
    async submitEditRoom() {
      ...
      try {
        ...
        const data = await this.apiPatch(`/api/rooms/${this.activeRoomId}`, {
          name,
          goal,
    ...
    async deleteRoom() {
      ...
      if (
        !confirm(
          '방을 삭제하면 모든 멤버가 퇴장되고 채팅 기록이 사라집니다. 정말 삭제하시겠습니까?',
        )
      )
        return;
      try {
    ...
    notifyTimerStart(subject) {
      ...
      const sock = getSocket();
      if (sock)
        sock.emit('room:timer_start', { roomId: this.activeRoomId, subject: subject || '공부' });
    },
    ...
    notifyTimerStop(durationSec) {
      ...
      const sock = getSocket();
      if (sock)
        sock.emit('room:timer_stop', { roomId: this.activeRoomId, duration_sec: durationSec || 0 });
    },
  };
