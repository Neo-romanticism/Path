...
(function () {
  ...
  document.addEventListener(
    'touchstart',
    function (e) {
      const a = e.target.closest('a[href], [data-nav-href], [onclick]');
      if (!a) return;
      let href = a.dataset && a.dataset.navHref;
      if (!href && a.href) {
        try {
  ...
  function runInlineStatement(stmt, event, element) {
    ...
    const args = argTokens.map(function (t) {
      return parseArg(t, event, element);
    });
  ...
  function detectInlineOnclickBlocked() {
    try {
      ...
      probe.setAttribute(
        'onclick',
        'window.__pathInlineProbe = (window.__pathInlineProbe || 0) + 1',
      );
      ...
      try {
        delete window.__pathInlineProbe;
      } catch (_) {}
    }
  }
    ...
    if (inlineBlocked) {
      document.addEventListener(
        ...
        function (e) {
          ...
          if (!target) return;
          const expr = target.getAttribute('onchange');
          if (!expr) return;
      ...
      document.addEventListener(
        ...
        function (e) {
          const target = e.target;
          if (!target) return;
          const expr = target.getAttribute('oninput');
          if (!expr) return;
          runInlineOnclick(expr, e, target);
        },
        { capture: true },
