...
import { create3DBalloon, setBalloonDetailLevel, getBalloonColors } from './balloonModel.js';

// ── World constants ──────────────────────────────────────────────────────────
// The game world is 1,000,000 × 1,000,000 world-units.  A WORLD_SCALE factor maps
...
const WORLD_SIZE        = 1000000; // total world width/height (world-units)
const WORLD_SCALE       = 15;      // scene-units per world-unit  (100x larger per coordinate)
...
const SEEDED_CULL_DIST  = 110000;  // distance culling for seeded scenery props
const CLOUD_CULL_DIST   = 150000;  // cloud-specific culling range
const SKY_CULL_DIST     = 70000;   // culling range for decorative sky islands
const PROP_CULL_DIST    = 85000;   // culling range for interactable props
...
const WorldScene = {
    ...
    gltfLoader: null,
    landmarkModelSceneCache: new Map(),
    landmarkModelPromiseCache: new Map(),
    ...
    init() {
        ...
        this.camera = new THREE.PerspectiveCamera(46, W / H, 1, 400000);
    ...
    _loadUniversityLandmarkModel(path) {
        if (this.landmarkModelSceneCache.has(path)) {
            return Promise.resolve(this.landmarkModelSceneCache.get(path));
        }
        if (this.landmarkModelPromiseCache.has(path)) {
            return this.landmarkModelPromiseCache.get(path);
        }
        ...
        const p = new Promise((resolve, reject) => {
            loader.load(
                ...
                (gltf) => {
                    ...
                    if (!scene) {
                        reject(new Error(`Invalid glTF scene for ${path}`));
                        return;
                    }
                    this.landmarkModelSceneCache.set(path, scene);
                    this.landmarkModelPromiseCache.delete(path);
                    resolve(scene);
                },
                undefined,
                (err) => {
        ...
        this.landmarkModelPromiseCache.set(path, p);
        return p;
    },

    _attachUniversityLandmarkModel(group, islandData, fallbackGroup) {
        const spec = this._getUniversityLandmarkModelSpec(islandData.university);
        if (!spec) return;

        this._loadUniversityLandmarkModel(spec.path)
            .then((sourceScene) => {
                if (!group.parent) return;
                const model = sourceScene.clone(true);
                model.traverse((obj) => {
                    if (!obj.isMesh) return;
                    obj.userData.propId = group.userData.propId;
                    if (obj.material) {
                ...
                if (fallbackGroup && fallbackGroup.parent === group) {
                    group.remove(fallbackGroup);
                }
            })
            .catch(() => {
                // Keep procedural fallback silently when model file is missing.
            });
    },
    ...
    _addUniversityCastle(group, islandData) {
        ...
        this._attachUniversityLandmarkModel(group, islandData, fallbackGroup);
    },
    ...
    /**
     * Deterministic even distribution across a 2D rectangle with mild jitter.
     * This avoids heavy random clustering while preserving a natural look.
     */
    _jitteredGridPoints(count, halfX, halfY, rng, jitter = 0.6) {
        if (count <= 0) return [];
        const cols = Math.ceil(Math.sqrt(count));
        const rows = Math.ceil(count / cols);
        const cellW = (halfX * 2) / cols;
        const cellH = (halfY * 2) / rows;
        const points = [];
        ...
        for (let i = 0; i < count; i++) {
            ...
            const x = -halfX + (col + 0.5) * cellW + jx;
            const y = -halfY + (row + 0.5) * cellH + jy;
            points.push({
        ...
        // Shuffle so placement order does not reveal row/column progression.
    ...
    /** Deterministic near-even line distribution with slight jitter. */
    _jitteredLinePoints(count, halfSpan, rng, jitter = 0.35) {
        if (count <= 0) return [];
        const points = [];
        const step = (halfSpan * 2) / count;
        for (let i = 0; i < count; i++) {
            const base = -halfSpan + (i + 0.5) * step;
            const x = base + (rng() - 0.5) * step * jitter;
            points.push(Math.max(-halfSpan, Math.min(halfSpan, x)));
        }
        for (let i = points.length - 1; i > 0; i--) {
            const j = Math.floor(rng() * (i + 1));
            [points[i], points[j]] = [points[j], points[i]];
    ...
    _updateDistanceCulling() {
        const seededCullSq = SEEDED_CULL_DIST * SEEDED_CULL_DIST;
        const cloudCullSq = CLOUD_CULL_DIST * CLOUD_CULL_DIST;
        const skyCullSq = SKY_CULL_DIST * SKY_CULL_DIST;
        const propCullSq = PROP_CULL_DIST * PROP_CULL_DIST;

        this.seededProps.forEach((obj) => {
            const kind = obj.userData.seedKind;
            const cullSq = kind === 'cloud' ? cloudCullSq : seededCullSq;
            const near = this._sqDistToCam(obj.position.x, obj.position.y) <= cullSq;
            obj.userData.nearCamera = near;
            if (kind === 'cloud') {
                obj.visible = near;
            } else {
                obj.visible = near;
        ...
        this.skyIslands.forEach((island) => {
            island.visible = this._sqDistToCam(island.position.x, island.position.y) <= skyCullSq;
        });

        this.interactableProps.forEach((prop) => {
            prop.group.visible = this._sqDistToCam(prop.group.position.x, prop.group.position.y) <= propCullSq;
        });
    },
    ...
    initSeed(seed) {
        ...
        // Increased count with deterministic even spread to reduce clustering.
        const CLOUD_SPREAD_X = WORLD_HALF * WORLD_SCALE * 0.85;
        const CLOUD_SPREAD_Y = 8000;
        const CLOUD_COUNT = 420;
        const cloudPoints = this._jitteredGridPoints(CLOUD_COUNT, CLOUD_SPREAD_X, CLOUD_SPREAD_Y * 0.5, rng, 0.7);
        const cloudTypes = ['normal', 'wispy', 'large', 'storm'];
        for (let i = 0; i < CLOUD_COUNT; i++) {
            const cx = cloudPoints[i].x;
            const cy = cloudPoints[i].y;
        ...
        const BUILDING_COUNT = 320;
        const buildingPoints = this._jitteredLinePoints(BUILDING_COUNT, BUILDING_SPREAD, rng, 0.45);
        for (let i = 0; i < BUILDING_COUNT; i++) {
            const bx = buildingPoints[i];
            ...
            mesh.userData.seedKind = 'building';
            this.scene.add(mesh);
        ...
        const ROCK_COUNT = 640;
        const rockPoints = this._jitteredGridPoints(ROCK_COUNT, ROCK_SPREAD, 2000, rng, 0.75);
        for (let i = 0; i < ROCK_COUNT; i++) {
            const rx = rockPoints[i].x;
            const ry = rockPoints[i].y;
        ...
        const CRYSTAL_COUNT = 180;
        const crystalPoints = this._jitteredGridPoints(CRYSTAL_COUNT, CRYSTAL_SPREAD, 1500, rng, 0.7);
        for (let i = 0; i < CRYSTAL_COUNT; i++) {
            const group = new THREE.Group();
            const cx2 = crystalPoints[i].x;
            const cy2 = crystalPoints[i].y;
            ...
            group.userData.seedKind = 'crystal';
            this.scene.add(group);
        ...
        const pillarPoints = this._jitteredLinePoints(PILLAR_COUNT, PILLAR_SPREAD, rng, 0.4);
        for (let i = 0; i < PILLAR_COUNT; i++) {
            const px = pillarPoints[i];
            const py = -80;
            ...
            mesh.userData.seedKind = 'pillar';
            this.scene.add(mesh);
            this.seededProps.push(mesh);
        }

        // ── Interactable sky-island portals ───────────────────────────────
        // Increased island count with even 2D distribution.
        const ISLAND_SPREAD = WORLD_HALF * WORLD_SCALE * 0.65;
        const ISLAND_COUNT = 96;
        const islandPoints = this._jitteredGridPoints(ISLAND_COUNT, ISLAND_SPREAD, 1250, rng, 0.8);
        const islandNames = [
            '별의 섬', '황금 섬', '구름 섬', '달빛 섬',
            '바람 섬', '불꽃 섬', '물결 섬', '숲의 섬',
            '얼음 섬', '번개 섬', '태양 섬', '은하 섬',
        ...
        for (let i = 0; i < ISLAND_COUNT; i++) {
            const wx = islandPoints[i].x;
            const wy = islandPoints[i].y;
    ...
    _updateBalloonColor(group, skinId) {
        ...
        const balloon3D = group.userData.balloon3D;
        if (!balloon3D) return;
    ...
    * worldX ∈ [-500000, 500000], worldY ∈ [-500000, 500000].
     */
    getWorldPosition() {
        return {
    ...
    _loop() {
        ...
        this.clouds.forEach((c, i) => {
            if (!c.userData.nearCamera) return;
        ...
        if (this.frameCount % 2 === 0) {
            this.seededProps.forEach(prop => {
                if (!prop.userData.nearCamera) return;
            ...
            this.balloons.forEach((b) => {
                ...
                if (dist > 6000) {
                    ...
                    if (b.group.userData.balloon3D) {
                        setBalloonDetailLevel(b.group.userData.balloon3D, dist > 2800);
                    }
        ...
        this.interactableProps.forEach(prop => {
            if (prop.group.visible) prop.update(t, this.camera);
        });
