...
function saveTimerUiSettings() {
  ...
  const nativeBrightnessReady =
    AppBrightnessManager.isNativeApp() && !!AppBrightnessManager.getPlugin();
  const powerSaveLabel = shouldPowerSave
    ? nativeBrightnessReady
      ? 'ON'
      : 'ON (앱 밝기 제어 미연결)'
    : 'OFF';
...
function ensureStudyHubProfileEditor() {
  if (
    _studyHubProfileEditor ||
    !window.PathProfileEditor ||
    typeof window.PathProfileEditor.create !== 'function'
  ) {
