const UI = {
  ...
  bindEvents() {
    const blockWhenStudying = (
      e,
      message = '타이머 진행 중에는 이동할 수 없습니다. 중단 후 이용해주세요.',
    ) => {
      if (typeof TimerEngine === 'undefined' || !TimerEngine.isActive || !TimerEngine.isActive())
        return false;
      if (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    const bindTabSwitch = (element, tab) => {
      this.bindTapAction(element, () => this.switchTab(tab));
    };
    const getCalendarBaseDate = () =>
      this.calendarSelectedDate || this.plannerDate || this.toLocalYmd(new Date());
    const getCalendarAnchorDate = () => this.calendarAnchorDate || getCalendarBaseDate();
    ...
    bindTabSwitch(this.elements.tabHomeBtn, 'home');
    bindTabSwitch(this.elements.tabStudyBtn, 'study');
    bindTabSwitch(this.elements.tabPlannerBtn, 'planner');
    bindTabSwitch(this.elements.tabCalendarBtn, 'calendar');
    bindTabSwitch(this.elements.tabScoreCalcBtn, 'scorecalc');
    bindTabSwitch(this.elements.tabBalloonBtn, 'balloon');
    bindTabSwitch(document.getElementById('tab-rooms-btn'), 'rooms');

    this.bindTapAction(
      this.elements.tabMoreBtn,
      (e) => {
        e.stopPropagation();
    ...
    this.bindTapAction(this.elements.tabCommunityBtn, (e) => {
      if (blockWhenStudying(e, '타이머 진행 중에는 커뮤니티로 이동할 수 없습니다.')) return;
    ...
    if (backBtn) {
      this.bindTapAction(backBtn, (e) => {
        if (blockWhenStudying(e, '타이머 진행 중에는 메인 허브로 나갈 수 없습니다.')) return;
        if (typeof window.navigateTo === 'function') {
    ...
    if (settingsBtn) {
      this.bindTapAction(settingsBtn, (e) => {
        if (blockWhenStudying(e, '타이머 진행 중에는 설정을 열 수 없습니다.')) return;
        if (typeof openTimerSettings === 'function') openTimerSettings();
      });
    }

    this.bindTapAction(this.elements.homeStartBtn, () => this.switchTab('study'));
    this.bindTapAction(this.elements.homePlanBtn, () => this.switchTab('planner'));
    this.bindTapAction(this.elements.homeRefreshBtn, () => {
      this.loadHomeHubData(true).catch(() => {});
    });
    this.bindTapAction(this.elements.homeBriefRoomsBtn, () => this.switchTab('rooms'));
    this.bindTapAction(this.elements.homeBriefSocialBtn, () => this.navigateToMessages());
    this.bindTapAction(this.elements.homeQuickApplyBtn, () => this.openHomeHubOverlay('apply'));
    this.bindTapAction(this.elements.homeQuickShopBtn, () => this.openHomeHubOverlay('shop'));
    this.bindTapAction(this.elements.homeQuickSocialBtn, () => this.navigateToMessages());
    this.bindTapAction(this.elements.homeQuickNotifBtn, () => this.openHomeHubOverlay('notif'));
    this.bindTapAction(this.elements.homeTopOpenBtn, () => this.openHomeHubOverlay('notif'));
    this.bindTapAction(this.elements.homeRoomOpenBtn, () => this.switchTab('rooms'));
    this.bindDelegatedTapAction(this.elements.homeTopList, '[data-home-action]', (e) =>
      this.handleHomeListAction(e),
    );
    this.bindDelegatedTapAction(this.elements.homeRoomList, '[data-home-action]', (e) =>
      this.handleHomeListAction(e),
    );
    this.bindTapAction(this.elements.hubOverlayCloseBtn, () => this.closeHomeHubOverlay());
    this.bindTapAction(this.elements.hubOverlayConfirmBtn, () => this.closeHomeHubOverlay());
    ...
    this.elements.scoreCalcTrack?.addEventListener('change', () =>
      this.renderScoreCalcFormulaHint(),
    );
    this.elements.scoreCalcCategory?.addEventListener('change', () =>
      this.renderScoreCalcFormulaHint(),
    );
    this.bindTapAction(this.elements.scoreCalcCalcBtn, () => this.calculateUniversityScore());

    this.bindTapAction(this.elements.modeTimer, () => this.setMode('timer'));
    ...
    this.bindDelegatedTapAction(
      this.elements.quickSubjects,
      '.quick-subject-btn[data-subject-name]',
      (e) => this.handleQuickSubjectClick(e),
    );
    ...
    this.bindDelegatedTapAction(
      ...
      (_e, btn) => {
        ...
        this.syncInputToDisplay();
        document
          .querySelectorAll('.time-preset-btn')
          .forEach((presetBtn) => presetBtn.classList.remove('active'));
    ...
    const planToggleArrow = document.getElementById('plan-toggle-arrow');
    const planSection = document.getElementById('plan-section');
    this.bindTapAction(planToggleBtn, () => {
      const willOpen = planSection?.classList.contains('hidden');
      planSection?.classList.toggle('hidden', !willOpen);
      planToggleBtn?.classList.toggle('open', Boolean(willOpen));
      if (planToggleArrow) planToggleArrow.textContent = willOpen ? '▴' : '▾';
    });

    this.elements.studyPlanForm?.addEventListener('submit', (e) => {
      e.preventDefault();
      this.handleAddStudyPlanDraft();
    });
    this.bindDelegatedTapAction(
      this.elements.studyPlanList,
      '.study-plan-delete[data-draft-id]',
      (e) => this.handleStudyPlanDraftDelete(e),
    );
    this.bindTapAction(this.elements.studyPlanSyncBtn, () => this.handleStudyPlanSyncClick());

    this.bindTapAction(this.elements.weekPrevBtn, () => {
      this.loadWeekCalendar(this.weekOffset - 1, {
        anchorDate: this.shiftDateByDays(getCalendarAnchorDate(), -7),
        selectedDate: this.shiftDateByDays(getCalendarBaseDate(), -7),
        syncPlanner: false,
      }).catch(() => {});
    });
    this.bindTapAction(this.elements.weekNextBtn, () => {
      this.loadWeekCalendar(this.weekOffset + 1, {
        anchorDate: this.shiftDateByDays(getCalendarAnchorDate(), 7),
        selectedDate: this.shiftDateByDays(getCalendarBaseDate(), 7),
        syncPlanner: false,
    ...
    this.bindTapAction(this.elements.planCancelBtn, () => {
      this.clearPlanFormEditingState();
      this.renderCalendarTimeline();
      this.renderCalendarDayPanel();
    });

    this.elements.calendarTimeline?.addEventListener('click', (e) =>
      this.handleCalendarTimelineClick(e),
    );
    this.elements.calendarDayPanel?.addEventListener('click', (e) =>
      this.handleCalendarDayPanelClick(e),
    );
    this.elements.calendarTimeline?.addEventListener('pointerdown', (e) =>
      this.handlePlanPointerDown(e),
    );
    ...
    this.bindTapAction(this.elements.calendarOpenPlannerBtn, () => this.switchTab('planner'));
    this.bindTapAction(this.elements.calendarTodayBtn, () => this.goToTodaySchedule());
    ...
    this.bindTapAction(this.elements.enterBtn, async () => {
      let hr = parseInt(this.elements.inputHr?.value, 10) || 0;
      let min = parseInt(this.elements.inputMin?.value, 10) || 0;
      ...
      this.elements.breakBtn.textContent =
        this.currentMode === 'stopwatch' ? '학습 완료하기' : '중단하기';

      if (Array.isArray(this.studyPlanDrafts) && this.studyPlanDrafts.length > 0) {
        const targetDate =
          this.plannerDate || this.calendarSelectedDate || this.toLocalYmd(new Date());
        this.persistStudyPlanDrafts(targetDate).catch((e) => {
      ...
      try {
        ...
        if (
          typeof window.GroupRooms !== 'undefined' &&
          typeof window.GroupRooms.notifyTimerStart === 'function'
        ) {
          const subjectName =
            this.elements.subjectSelect?.options[this.elements.subjectSelect.selectedIndex]?.text ||
            '공부';
          window.GroupRooms.notifyTimerStart(subjectName);
        }
      } catch (e) {
    ...
    this.bindTapAction(this.elements.breakBtn, () => {
    ...
    this.elements.proofFileInput?.addEventListener('change', () =>
      this.handleStudyProofSelection(),
    );
    this.bindTapAction(this.elements.proofUploadBtn, () => this.uploadStudyProof());
    this.bindTapAction(this.elements.resetBtn, () => location.reload());
  },

  bindTapAction(element, handler, options = {}) {
  ...
  bindDelegatedTapAction(container, selector, handler, options = {}) {
  ...
  openHomeRoom(roomId) {
    ...
    if (
      typeof window.GroupRooms !== 'undefined' &&
      typeof window.GroupRooms.openRoom === 'function'
    ) {
  ...
  renderHomeLoadingState() {
    ...
    if (this.elements.homeTopList) this.elements.homeTopList.innerHTML = skeleton;
    if (this.elements.homeRoomList) this.elements.homeRoomList.innerHTML = skeleton;
  ...
  renderHomeErrorState(err) {
    ...
    const retry =
      '<button type="button" class="home-row-cta home-row-cta-wide" data-home-action="retry-home">다시 불러오기</button>';
    if (this.elements.homeTopList) {
    ...
    if (this.elements.homeRoomList) {
      this.elements.homeRoomList.innerHTML =
        '<div class="home-empty home-empty-stack"><strong>공개 그룹을 불러오지 못했습니다</strong><span>잠시 후 다시 시도하거나 그룹 탭에서 직접 확인하세요.</span><button type="button" class="home-row-cta home-row-cta-wide" data-home-action="browse-rooms">그룹 탭 열기</button></div>';
    }
    if (this.elements.homeHeroTitle)
      this.elements.homeHeroTitle.textContent = '홈 요약을 다시 불러오는 중입니다';
    if (this.elements.homeHeroCopy)
      this.elements.homeHeroCopy.textContent =
        '네트워크 상태를 확인한 뒤 새로고침하면 최신 홈 데이터를 다시 받아옵니다.';
  ...
  updateHomeHeroSummary({ todaySec, totalSec, activeCount, ranking, rooms }) {
    ...
    const rankPct = Number.isFinite(Number(rankPctRaw))
      ? `${Number(rankPctRaw).toFixed(2)}%`
      : null;
    const roomCount = Array.isArray(rooms) ? rooms.length : 0;
    const studyingUsers = Array.isArray(ranking)
      ? ranking.filter((row) => row?.is_studying).length
      : 0;
    const activeRoom = Array.isArray(rooms)
      ? rooms.find((room) => (parseInt(room?.member_count, 10) || 0) > 0)
      : null;

    if (this.elements.homeHeroTier) this.elements.homeHeroTier.textContent = tier;
    if (this.elements.homeHeroRank)
      this.elements.homeHeroRank.textContent = rankPct ? `상위 ${rankPct}` : '오늘 순위 집계 중';
    if (this.elements.homeHeroTitle) {
      this.elements.homeHeroTitle.textContent =
        todaySec > 0
    ...
    if (this.elements.homeNextFocus) {
      this.elements.homeNextFocus.textContent =
        todaySec >= 5400
          ? '계획을 추가하고 다음 블록으로 이어가기'
          : '학습 탭으로 이동해 바로 시작하기';
    }
    if (this.elements.homeNextFocusMeta) {
      this.elements.homeNextFocusMeta.textContent =
        todaySec >= 5400
    ...
    if (this.elements.homeActiveSummary) {
      this.elements.homeActiveSummary.textContent =
        studyingUsers > 0 ? `지금 ${studyingUsers}명이 공부 중입니다` : '지금은 조용한 구간입니다';
    }
  ...
  switchTab(tab) {
    ...
    this.elements.tabMoreBtn?.setAttribute(
      'aria-label',
      isMoreActive ? `더보기 (${this.currentTab})` : '더보기',
    );
    this.setMoreMenuOpen(false);
  ...
  setMoreMenuOpen(open) {
    ...
    this.moreMenuGuardRaf = requestAnimationFrame(() => {
      this.moreMenuGuardRaf = requestAnimationFrame(() => {
        ...
        const menuVisible =
          !this.elements.tabMoreMenu.classList.contains('hidden') &&
  ...
  async loadHomeHubData(forceReload = false) {
    ...
    if (forceReload || !this.homeLastUpdatedAt) {
    ...
    try {
      ...
      const activeCount = rankingRows.reduce((acc, row) => acc + (row?.is_studying ? 1 : 0), 0);

      if (this.elements.homeStatToday)
        this.elements.homeStatToday.textContent = this.formatDuration(todaySec);
      if (this.elements.homeStatTodayMeta)
        this.elements.homeStatTodayMeta.textContent =
          todaySec > 0
            ? `지금까지 ${this.formatDuration(todaySec)} 진행했습니다`
            : '지금 25분만 시작해도 흐름이 붙습니다';
      if (this.elements.homeStatTotal)
        this.elements.homeStatTotal.textContent = this.formatDuration(totalSec);
      if (this.elements.homeStatTotalMeta)
        this.elements.homeStatTotalMeta.textContent =
          totalSec > 0 ? '누적 집중이 계속 쌓이고 있습니다' : '첫 기록을 만들 준비가 됐습니다';
      if (this.elements.homeStatActive)
        this.elements.homeStatActive.textContent = `${activeCount}명`;
      if (this.elements.homeStatActiveMeta)
        this.elements.homeStatActiveMeta.textContent =
          activeCount > 0
            ? '지금 바로 자극을 받을 수 있습니다'
            : '조용한 시간대지만 먼저 시작할 수 있습니다';

      this.homeLastUpdatedAt = Date.now();
      this.updateHomeHeroSummary({
        todaySec,
        totalSec,
        activeCount,
        ranking: rankingRows,
      ...
      this.renderHomeTopRanking(rankingRows.slice(0, 5));
      this.renderHomePublicRooms(publicRooms);
    } catch (err) {
  ...
  renderHomeTopRanking(rows) {
    ...
    if (!Array.isArray(rows) || rows.length === 0) {
      this.elements.homeTopList.innerHTML =
        '<div class="home-empty home-empty-stack"><strong>아직 오늘 랭킹 데이터가 없습니다</strong><span>첫 세션을 시작하면 홈 랭킹 흐름도 함께 살아납니다.</span><button type="button" class="home-row-cta home-row-cta-wide" data-home-action="start-study">지금 학습 시작</button></div>';
      return;
    }

    this.elements.homeTopList.innerHTML = rows
      .map((row, idx) => {
  ...
  renderHomePublicRooms(rooms) {
    ...
    if (!Array.isArray(rooms) || rooms.length === 0) {
      this.elements.homeRoomList.innerHTML =
        '<div class="home-empty home-empty-stack"><strong>현재 공개 그룹이 없습니다</strong><span>먼저 개인 학습을 시작하거나 그룹 탭에서 새 방을 만들 수 있습니다.</span><button type="button" class="home-row-cta home-row-cta-wide" data-home-action="browse-rooms">그룹 탭 열기</button></div>';
      return;
    }

    this.elements.homeRoomList.innerHTML = rooms
      .map((room) => {
  ...
  openHomeHubOverlay(type) {
    if (
      !this.elements.hubOverlay ||
      !this.elements.hubOverlayTitle ||
      !this.elements.hubOverlayBody
    )
      return;
  ...
  async renderHubApplyPanel() {
    ...
    this.elements.hubOverlayBody
      .querySelector('#hub-apply-refresh-btn')
    ...
    this.elements.hubOverlayBody
      .querySelector('#hub-apply-open-btn')
      ?.addEventListener('click', () => {
  ...
  async renderHubShopPanel() {
    ...
    this.elements.hubOverlayBody
      .querySelector('#hub-shop-collect-btn')
      ?.addEventListener('click', async (e) => {
        ...
        try {
          ...
          alert(
            data?.collected > 0
              ? `${data.collected.toLocaleString()}G 수령 완료`
              : data?.message || '수령할 수입이 없습니다.',
          );
    ...
    this.elements.hubOverlayBody
      .querySelector('#hub-shop-buy-ticket-btn')
      ?.addEventListener('click', async (e) => {
        const btn = e.currentTarget;
        const prev = btn.textContent;
        btn.disabled = true;
    ...
    this.elements.hubOverlayBody
      .querySelectorAll('.hub-shop-action-btn, .hub-shop-quick-btn')
      .forEach((btn) => {
  ...
  async handleHubShopAction({ kind, id, mode, button, autoEquip = false }) {
    ...
    button.textContent =
      kind === 'collect' ? '수령중...' : mode === 'equip' ? '장착중...' : '구매중...';
  ...
  async renderHubSocialPanel() {
    ...
    if (!this.hubSocialCurrentChatId) {
      ...
      this.hubSocialCurrentChatNickname = String(
        firstConvo?.nickname || firstFriend?.nickname || '',
      );
    }
    ...
    this.elements.hubOverlayBody
      .querySelector('#hub-social-refresh-btn')
      ?.addEventListener('click', () => {
  ...
  async renderHubSocialConversation() {
    ...
    if (!messages.length) {
      listEl.innerHTML =
        '<div class="hub-social-empty-chat">아직 주고받은 메시지가 없습니다.</div>';
      return;
    }
      ...
      .map((c) => {
        ...
        const unreadBadge =
          Number(c?.unread_count || 0) > 0
            ? `<em>${Math.min(99, Number(c.unread_count))}</em>`
            : '';
  ...
  async renderHubNotifPanel() {
    ...
    this.elements.hubOverlayBody
      .querySelector('#hub-notif-refresh-btn')
      ?.addEventListener('click', () => {
        this.renderHubNotifPanel().catch((err) => {
          this.elements.hubOverlayBody.innerHTML = `<div class="hub-apply-error">${this.escapeHtml(err?.message || '새로고침 실패')}</div>`;
        });
    ...
    this.elements.hubOverlayBody
      .querySelector('#hub-notif-readall-btn')
      ?.addEventListener('click', async (e) => {
        const btn = e.currentTarget;
        const prev = btn.textContent;
        btn.disabled = true;
  ...
  async loadUniversityCalculator(preferredUniversity) {
    ...
    this.elements.scoreCalcUniversity.innerHTML = [
      ...
      ...this.universityList.map(
        (u) => `<option value="${this.escapeHtml(u.name)}">${this.escapeHtml(u.name)}</option>`,
      ),
    ].join('');
  ...
  async handleScoreCalcUniversityChange() {
    ...
    this.elements.scoreCalcTrack.innerHTML = tracks
      .map(
        (track, idx) =>
          `<option value="${this.escapeHtml(track)}"${idx === 0 ? ' selected' : ''}>${this.escapeHtml(track)}</option>`,
      )
      .join('');
  ...
  populateScoreCalcCategories() {
    ...
    const categories = [
      ...new Set(
        (this.scoreCalcUniversityInfo?.departments || [])
  ...
  calculateUniversityScore() {
    ...
    if (total == null) {
      this.elements.scoreCalcResult.textContent =
        '입력값을 다시 확인해주세요. (필수 영역 점수 누락 또는 범위 오류)';
      return;
    }
  ...
  buildScoreComparisonHtml(universityInfo, total) {
    ...
    const rows = (universityInfo?.departments || [])
      .map((dept) => {
        ...
        const bestCut = candidates.sort(
          (a, b) => Math.abs(total - a.cut) - Math.abs(total - b.cut),
        )[0];
  ...
  renderBalloonAiReport({ stats, rankInfo, user }) {
    if (
      !this.elements.balloonAiHeadline ||
      !this.elements.balloonAiDetail ||
      !this.elements.balloonAiMeta
    ) {
      return;
    }
    ...
    const estimatedNow = this.estimateAcceptanceChance(
      userPercentile,
      targetBase,
      totalSec,
      successSec,
    );
    const predictedLift = this.estimateTrendLift(
      userPercentile,
      targetBase,
      todaySec,
      totalSec,
  ...
  pickTargetUniversity(userPercentile, preferredUniversity) {
    ...
    if (
      !Array.isArray(this.universityList) ||
    ...
    const sorted = [...this.universityList]
      ...
      .sort(
        (a, b) =>
          Math.abs(Number(a.basePercentile) - userPercentile) -
          Math.abs(Number(b.basePercentile) - userPercentile),
      );
  ...
  renderQuickSubjects() {
    ...
    this.elements.quickSubjects.innerHTML = this.quickSubjectNames
      .map((name) => {
  ...
  renderStudyPlanDrafts() {
    ...
    this.elements.studyPlanList.innerHTML = this.studyPlanDrafts
      .map((item, idx) => {
  ...
  async persistStudyPlanDrafts(targetDate = this.toLocalYmd(new Date()), options = {}) {
    ...
    let cursor = existingPlans.reduce(
      (maxMinute, plan) => {
    ...
    for (const draft of this.studyPlanDrafts) {
      ...
      const duration = Math.max(
        snap,
        Math.min(300, parseInt(draft.durationMin, 10) || this.getDefaultPlanDuration()),
      );
  ...
  async loadWeekCalendar(offset = 0, options = {}) {
    ...
    const anchorDate =
      this.normalizeDateKey(options.anchorDate) ||
      this.calendarAnchorDate ||
      this.calendarSelectedDate ||
      this.plannerDate ||
      this.toLocalYmd(new Date());
    const selectedDate =
      this.normalizeDateKey(options.selectedDate) ||
      this.calendarSelectedDate ||
      this.plannerDate ||
      anchorDate;
  ...
  renderWeekLabel() {
    ...
    const offsetText =
      this.weekOffset === 0 ? '이번 주' : `${this.weekOffset > 0 ? '+' : ''}${this.weekOffset}주`;
  ...
  renderCalendarTimeline() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const totalRecordSec = records.reduce(
        (acc, r) => acc + (parseInt(r.duration_sec, 10) || 0),
        0,
      );
      ...
      const planItems = plans
        .map((p) => {
          ...
          const height = Math.max(
            22,
            Math.floor(((clampedEnd - clampedStart) / totalMinutes) * timelineHeight),
          );
    ...
    const legendHtml = Array.from(usedSubjects.values())
      .map((s) => {
  ...
  getScheduleDayData(dateText, sourceData = this.weekData) {
    ...
    if (!dateKey || !sourceData) {
      return {
        dateKey: this.normalizeDateKey(dateText),
        plans: [],
        records: [],
        plannedMin: 0,
        actualSec: 0,
    ...
    const plans = (sourceData.plans || []).filter(
      (plan) => this.normalizeDateKey(plan.plan_date) === dateKey,
    );
    const records = (sourceData.records || []).filter(
      (record) => this.normalizeDateKey(record.record_date || record.created_at) === dateKey,
    );
    ...
    const actualSec = records.reduce(
      (acc, record) => acc + (parseInt(record.duration_sec, 10) || 0),
      0,
    );
  ...
  renderCalendarDayPanel() {
    ...
    const selectedLabel = this.formatLongDate(
      dayData.dateKey || this.calendarSelectedDate || this.toLocalYmd(new Date()),
    );
    const summaryText =
      dayData.plans.length > 0 || dayData.records.length > 0
        ? `계획 ${this.formatDuration(dayData.plannedMin * 60)} · 실제 ${this.formatDuration(dayData.actualSec)}`
        : '아직 일정이나 공부 기록이 없습니다.';

    if (this.elements.calendarSelectedDateLabel)
      this.elements.calendarSelectedDateLabel.textContent = selectedLabel;
    if (this.elements.calendarSelectedDateSummary)
      this.elements.calendarSelectedDateSummary.textContent = summaryText;
  ...
  handlePlanPointerMove(event) {
    ...
    if (this.dragState.mode === 'resize') {
      ...
      nextStart = Math.max(
        minStart,
        Math.min(maxStart, this.dragState.originalStart + deltaMinute),
      );
      nextEnd = nextStart + this.dragState.duration;

      const deltaDayRaw =
        (event.clientX - this.dragState.startX) / Math.max(40, this.dragState.colWidth);
      const deltaDay = Math.round(deltaDayRaw);
      nextDayIndex = Math.max(0, Math.min(6, this.dragState.dayIndex + deltaDay));
    }

    if (
      nextStart === this.dragState.nextStart &&
      nextEnd === this.dragState.nextEnd &&
      nextDayIndex === this.dragState.nextDayIndex
    )
      return;
    ...
    const top = Math.floor(
      ((nextStart - this.timetableConfig.dayStartMinute) / totalMinutes) * timelineHeight,
    );
    ...
    if (this.dragState.mode === 'resize') {
      const nextHeight = Math.max(
        22,
        Math.floor(((nextEnd - nextStart) / totalMinutes) * timelineHeight),
      );
    ...
    if (timeEl)
      timeEl.textContent = `${this.minuteToTime(nextStart)} - ${this.minuteToTime(nextEnd)}`;
  },
  ...
  async handlePlanDelete(event) {
    const btn = event.target.closest(
      '.plan-delete-btn[data-plan-id], .calendar-day-delete[data-plan-id]',
    );
    if (!btn) return;
  ...
  startEditingPlan(planId, options = {}) {
    ...
    const planDate =
      this.normalizeDateKey(plan.plan_date) ||
      this.calendarSelectedDate ||
      this.toLocalYmd(new Date());
    ...
    if (this.elements.planSubjectSelect)
      this.elements.planSubjectSelect.value = String(plan.subject_id || '');
    if (this.elements.planDate) this.elements.planDate.value = planDate;
    if (this.elements.planStartTime)
      this.elements.planStartTime.value = this.minuteToTime(parseInt(plan.start_minute, 10) || 0);
    if (this.elements.planEndTime)
      this.elements.planEndTime.value = this.minuteToTime(parseInt(plan.end_minute, 10) || 0);
  ...
  clearPlanFormEditingState(nextDate = null) {
    ...
    const targetDate =
      this.normalizeDateKey(nextDate) ||
      this.calendarSelectedDate ||
      this.plannerDate ||
      this.toLocalYmd(new Date());
    if (this.elements.planDate) this.elements.planDate.value = targetDate;
    if (this.elements.planStartTime && !this.elements.planStartTime.value)
      this.elements.planStartTime.value = '09:00';
    if (this.elements.planEndTime && !this.elements.planEndTime.value)
      this.elements.planEndTime.value = '11:00';
    ...
    if (this.elements.planFormStatus) {
      this.elements.planFormStatus.textContent =
        '주간 계획이나 수업 일정을 추가하고 드래그로 옮길 수 있습니다.';
    }
  },
  ...
  findPlanById(planId) {
    ...
    return (
      (this.weekData?.plans || []).find((plan) => Number(plan.id) === numericId) ||
  ...
  updateAssets(data) {
    ...
    if (this.elements.diamondVal)
      this.elements.diamondVal.innerText = (data.diamond || 0).toLocaleString();
    if (this.elements.ticketVal)
      this.elements.ticketVal.innerText = String(data.tickets || 0).padStart(2, '0');
  ...
  showResult(type, gold = 0, mode = 'timer', studyRecordId = null) {
    ...
    if (type === 'SUCCESS') {
      ...
      this.elements.resLoot.innerHTML = `<span style="color:var(--accent);font-size:1.1rem">이탈이 감지되었습니다.</span>`;
    }
  },
  ...
  bindPlannerEvents() {
    ...
    document
      .getElementById('planner-dday-save')
      ?.addEventListener('click', () => this.savePlannerDday());
    document
      .getElementById('planner-memo-save')
      ?.addEventListener('click', () => this.savePlannerMemo());
  ...
  _getPlannerDayData() {
    ...
    const plans = (this.plannerData.plans || []).filter(
      (p) => this.normalizeDateKey(p.plan_date) === dateKey,
    );
    const records = (this.plannerData.records || []).filter(
      (r) => this.normalizeDateKey(r.record_date || r.created_at) === dateKey,
    );
  ...
  renderPlannerTimeline() {
    ...
    const recordBlocks = records
      .map((r) => {
        ...
        const height = Math.max(
          20,
          Math.floor(((clampedEnd - startMin) / totalMinutes) * timelineHeight),
        );
  ...
  renderPlannerSubjectSummary() {
    ...
    chipsEl.innerHTML = Array.from(subjectMap.values())
      .map((s) => {
  ...
  async handlePlannerAddPlan() {
    ...
    try {
      ...
      if (document.getElementById('planner-add-note'))
        document.getElementById('planner-add-note').value = '';
      await this.loadPlannerData();
      await this.loadWeekCalendar(0, {
        anchorDate: this.plannerDate,
        selectedDate: this.plannerDate,
